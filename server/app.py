...
@app.route('/')
def index():
    """Main dashboard page with proper error handling"""
    try:
        ...
        # Try to get real data if sync engine is available
        if sync_engine:
            try:
...
def setup():
    ...
    try:
        ...
        for account in accounts['google']:
            stored_creds = config.get_credentials(account['id'])
            account['authenticated'] = stored_creds and 'google_token' in stored_creds

        for account in accounts['apple']:
            stored_creds = config.get_credentials(account['id'])
            account['authenticated'] = stored_creds and 'app_password' in stored_creds
...
def setup_google():
    ...
    return render_template('google_setup.html', error=None)
...
def setup_apple():
    ...
    return render_template('apple_setup.html', error=None)
...
@app.route('/display')
def display_view():
    """Calendar display page for Pi Zero"""
